...
"""
simple_gui.py
This module provides a simple Tkinter GUI to control the ultraArm P1
...
class Window:
    ...
    def need_input(self):
        ...
        # Coordinate labels
        coord_names = ["x", "y", "z", "rx"]
        for i, name in enumerate(coord_names):
            tk.Label(self.frmRT, text=f" {name} ").grid(row=i)

        # Initialize StringVars for joint and coordinate inputs
        self.j_vars = [tk.StringVar(value=str(val)) for val in self.res_angles[:4]]
        self.c_vars = [tk.StringVar(value=str(val)) for val in self.record_coords[:4]]
    ...
    def show_init(self):
        ...
        self.cont_vars = [tk.StringVar(value=f"{val}°") for val in self.res_angles[:4]]
        ...
        self.coord_vars = [tk.StringVar(value=str(val)) for val in self.record_coords[:4]]
        coord_names = ["x", "y", "z", "rx"]
    ...
    def update_display(self):
        ...
        for val, var in zip(self.res_angles, self.cont_vars):
            var.set(f"{val}°")
        for val, var in zip(self.record_coords[:4], self.coord_vars):
            var.set(str(val))
    ...
    def get_date(self):
        ...
        try:
            ...
            if res:
                self.record_coords = [
                    round(res.x, 2), round(res.y, 2), round(res.z, 2), round(res.rx, 2), self.speed]
            else:
                self.record_coords = [-1] * 4
            # get joint angles
            angles = self.get_angles()
            if angles:
                self.res_angles = [
                    round(angles.joint_1, 2), round(angles.joint_2, 2), round(angles.joint_3, 2),
                    round(angles.joint_4, 2)]
            else:
                self.res_angles = [-1] * 4
        except ServiceException:
            print("ROS service unavailable, skip update.")
